...
const routerExtension = (pi: ExtensionAPI) => {
  ...
  const persistState = () => {
    // In-flight guard: skip if already persisting to avoid duplicate writes.
  ...
  const actions = {
    ...
    reloadConfig: (
      ...
      lastConfigPath = loaded.configPath;
    ...
    switchToRouterProfile: async (
      ...
      if (!currentConfig.profiles[profileName]) {
        if (strict) {
          ctx.ui.notify(
            `Unknown router profile: ${profileName}`,
            "error",
          );
        }
        return false;
      ...
      if (!success) {
        ctx.ui.notify(
          `Failed to switch to router/${profileName}`,
          "error",
        );
        return false;
      }
  ...
  const restoreStateFromSession = async (
    ...
    let savedState: RouterPersistedState | undefined;
    for (let i = routerEntries.length - 1; i >= 0; i--) {
      const entry = routerEntries[i];
      if (entry && isRouterPersistedState(entry.data)) {
  ...
  pi.on("model_select", async (event, ctx) => {
    ...
    if (event.model.provider === "router") {
      ...
      if (!profileName) {
        ctx.ui.notify(
          `Unknown router profile: ${event.model.id}`,
          "error",
        );
        return;
      }
  ...
  pi.on("thinking_level_select", (event, ctx) => {
    if (!isInitialized || !routerEnabled || !selectedProfile)
      return;
    ...
    actions.updateStatus(ctx);
    if (event.level !== "off") {
      const profile = currentConfig.profiles[selectedProfile];
      if (profile) {
        const unsupported = getUnsupportedTiers(profile, event.level);
