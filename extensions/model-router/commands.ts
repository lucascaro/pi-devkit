...
import { ROUTER_PIN_VALUES } from "./constants.js";
import {
  formatPinSummary,
...
export const registerCommands = (
  ...
  state: {
    ...
    readonly lastConfigPath: string | undefined;
  },
  actions: {
    persistState: () => void;
  ...
  const SUBCOMMAND_DETAILS = [
    { name: "init", desc: "Create a model-router config from the example" },
    { name: "enable", desc: "Enable the router with the default profile" },
    ...
    {
      name: "thinking",
      desc: "Override thinking level for a tier or profile",
    },
  ...
  const getThinkingCompletions = (
    ...
    const arg0 = args[0];
    if (arg0 && levelValues.includes(arg0)) {
      return null;
    }

    if (arg0 && (tierValues as string[]).includes(arg0)) {
      const tier = arg0;
  ...
  const handleInit = async (
    ...
    try {
      ...
      ctx.ui.notify(
        "Could not find the example config file. Please create ~/.pi/model-router.json manually.",
        "error",
      );
      return;
    }

    const targetPath = args[0] ?? join(getAgentDir(), "model-router.json");

    try {
      if (readFileSync(targetPath, "utf8")) {
        ctx.ui.notify(
          `Config already exists at ${targetPath}. Remove it first if you want to overwrite.`,
          "warning",
        );
        return;
      }
    ...
    try {
      ...
      ctx.ui.notify(
        `Config written to ${targetPath}. Run /router reload to activate.`,
        "info",
      );
  ...
  const handleEnable = async (
    ...
    if (args.length > 1) {
      ctx.ui.notify(
        "Usage: /router enable [profile]",
        "error",
      );
      return;
    }
    const profileName = args[0] ?? "balanced";
    const success = await actions.switchToRouterProfile(
      profileName,
      ctx,
    );
    if (success) {
  ...
  const handleStatus = async (
    ...
    const lines = [
      ...
      `Selected profile pin: ${
        state.selectedProfile
          ? state.pinnedTierByProfile[state.selectedProfile] ??
            "auto"
          : "none"
      }`,
      ...
      `Session cost: $${state.accumulatedCost.toFixed(4)}` +
        (state.currentConfig.maxSessionBudget
          ? ` / $${state.currentConfig.maxSessionBudget.toFixed(
              2,
            )}`
          : ""),
    ...
    if (
      ...
      state.lastConfigWarnings.length > 0
    ) {
      lines.push(
        "",
    ...
    if (args.length === 0) {
      ctx.ui.notify(
        [
          ...
          `Pinned tier: ${
            state.pinnedTierByProfile[currentProfile] ?? "auto"
          }`,
  ...
  const handleThinking = async (
    ...
    if (args.length === 0) {
      ctx.ui.notify(
        [
          `Profile: ${currentProfile}`,
          `Thinking overrides: ${JSON.stringify(
    ...
    if (args.length === 1) {
      ...
      const arg0 = args[0] ?? "";
      if (
        tierValues.includes(arg0) ||
        arg0 === "all"
      ) {
        tier = arg0 as RouterTier | "all";
        levelValue = args[1] ?? "";
      } else {
        ctx.ui.notify(
    ...
    if (tier !== "all" && !(tierValues as string[]).includes(tier as string)) {
      ctx.ui.notify(
    ...
    if (tier === "all") {
      for (const t of ROUTER_TIERS) {
        if (!state.thinkingByProfile[currentProfile])
          state.thinkingByProfile[currentProfile] = {};
        if (nextLevel)
          state.thinkingByProfile[currentProfile]![t] = nextLevel;
    ...
    if (nextLevel && nextLevel !== "off") {
      const profile = state.currentConfig.profiles[currentProfile];
      if (profile) {
        const unsupported = getUnsupportedTiers(profile, nextLevel);
  ...
  const handleDisable = async (
    ...
    if (provider === "router") {
      ctx.ui.notify(
        `Recorded model is a router model (${state.lastNonRouterModel}). ` +
          "Use /model to pick a concrete model instead.",
        "warning",
      );
      return;
    }
  ...
  const handleFix = async (
    ...
    const profile =
      state.currentConfig.profiles[state.lastDecision.profile];
    if (!profile?.[tier as RouterTier]) {
      ctx.ui.notify(
        `Profile "${state.lastDecision.profile}" does not have a "${tier}" tier configured. Pinning has no effect.`,
        "warning",
      );
      return;
    }
  ...
  const handleWidget = async (
    ...
    ctx.ui.notify(
      `Router widget ${
        state.widgetEnabled ? "enabled" : "disabled"
      }.`,
      "info",
    );
  };
  ...
  const handleDebug = async (
    ...
    if (cmd === "on") state.debugEnabled = true;
      ...
      // No argument or unrecognized: toggle as default fallback.
      state.debugEnabled = !state.debugEnabled;
    }
    actions.persistState();
    ctx.ui.notify(
  ...
  const handleReload = async (
    ...
    ctx.ui.notify(
      `Router config reloaded. Profiles: ${profileNames(
        state.currentConfig,
      ).join(", ")}`,
      "info",
    );
  ...
  pi.registerCommand("router", {
    ...
    getArgumentCompletions: (prefix) => {
      ...
      switch (subcommand) {
        case "init": {
          const pathPrefix = subArgs[0] ?? "";
          if (pathPrefix) {
            return [{
              value: `init ${pathPrefix}`,
              label: `init ${pathPrefix}`,
              description: `Write config to ${pathPrefix}`,
            }];
          }
          return [{
            value: "init",
            label: "init",
            description: "Create config at ~/.pi/model-router.json",
          }];
        }
        case "enable": {
          ...
          const items = profileNames(state.currentConfig)
            ...
            .map((name) => ({
              value: `enable ${name}`,
              label: `router/${name}`,
              description: `Enable router with profile "${name}" (default: balanced)`,
            }));
          return items.length > 0 ? items : null;
        }
        case "profile": {
        ...
        case "pin": {
          ...
          return (
            completions?.map((c) => ({
              ...
              value: `pin ${c.value}`,
              description:
                c.description ?? `Pin routing to ${c.label}`,
            })) ?? null
          );
        }
        case "thinking": {
    ...
    handler: async (args, ctx) => {
      ...
      switch (subcommand) {
        case "init":
          ...
          break;
        case "enable":
          await handleEnable(subArgs, ctx);
          break;
        case "profile":
        ...
        case "?":
          ...
          ctx.ui.notify(
            [
              "Router Subcommands:",
              "  init [path]                 Create a config file from the example (default: ~/.pi/model-router.json).",
              "  enable [profile]            Enable the router (default profile: balanced).",
        ...
        default:
          if (subcommand) {
            if (state.currentConfig.profiles[subcommand]) {
              ...
              await actions.switchToRouterProfile(
                subcommand,
                ctx,
              );
              ctx.ui.notify(
